...
static void set_action_icons(int inSystemState)
{
	...
	struct
	{
		...
		//               up                              center                           down
		/*initial */ {&s_settings_bitmap, &plannedCP},
		{&s_empty_bitmap, &emptyCP},
		{&s_play_bitmap, &initialStartCP},
		/*running */ {&s_pause_bitmap, &setPausedCP},
		{&s_empty_bitmap, &addLapsCP},
		{&s_next_bitmap, &runningStartCP},
		/*paused  */ {&s_empty_bitmap, &emptyCP},
		{&s_cross_bitmap, &clearDataCP},
		{&s_play_bitmap, &resetPausedCP},
		/*finished*/ {&s_empty_bitmap, &emptyCP},
		{&s_cross_bitmap, &clearDataCP},
		{&s_info_bitmap, &runningStartCP},
	};
...
void addTextLayers(int inWindowIndex)
{
	...
	for (int layerIndex = 0; layerIndex < NUM_LAYERS; layerIndex++, layerPointer++)
	{
		...
		int textYCoord = layerPointer->drawHeightTimes * drawHeightWithoutPointer / layerPointer->drawHeightDividedBy + POINTER_WID - layerPointer->textHeight / 2;
